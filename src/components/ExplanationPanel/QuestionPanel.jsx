    ...
    return (
        <div className="question-panel-container">
            <div className="question-list">
                {questions.map((q, idx) => {
                    return (
                        <div key={idx} className="question-container" onClick={() => props.onQuestionAsked()}>
                            {q}
                        </div>
                    );
                })}
...
function getQuestions(results, activity, predActivity) {
    let resTypes = Object.keys(results);

    let thisThese = "this";
    let sample = "sample is";
    if (results[resTypes].length > 1) {
        thisThese = "these";
        sample = "samples are";
    }

    let q = "";
    if ("TPFP".includes(resTypes[0])) {
        q = (
