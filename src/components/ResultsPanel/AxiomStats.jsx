...
function TimeDistanceAxiomStat(props) {
	const { stats, axiom } = props;
	const { minDuration1, minDuration2, maxDuration1, maxDuration2, minTimeDistance, maxTimeDistance } = stats;

	return (
		<div className="td-stat-container">
			<DurationStat key={"duration1"} event={"Fridge"} mind={minDuration1} maxd={maxDuration1}></DurationStat>
			<TimeDistanceStat key={"td-s"} tdmax={maxTimeDistance} tdmin={minTimeDistance}></TimeDistanceStat>
			<DurationStat key={"duration2"} event={"Cabinet"} mind={minDuration2} maxd={maxDuration2}></DurationStat>
		</div>
	);
}
	...
	return (
		<div className="timed-istance-stat-container">
			<svg width={180} height={70}>
				<line x1={0} y1={10 + 22} x2={180} y2={10 + 22} stroke="#5F5656"></line>
