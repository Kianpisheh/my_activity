	...
	items = (
		<ul>
			{actRuleitems.map((rule, idx) => {
				...
				return (
					<li key={idx}>
						<div key={idx + "ruleitem-div"} className="ruleitem-div">
							<div key={idx + "item-icons-div"} className="item-icons-div">
								{rule["items"].map((item, idx2) => {
									const Icon = Icons.getIcon(pascalCase(item));
							...
							<div className="all-exp-div">
								...
								<div className="rule-suppconf-exp">
									...
									>
										Accuracy: {rule["supp"] + " / " + Math.round(rule["supp"] / rule["conf"])}
									</span>
